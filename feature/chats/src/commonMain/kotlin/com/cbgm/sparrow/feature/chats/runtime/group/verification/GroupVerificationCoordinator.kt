...
class GroupVerificationCoordinator internal constructor(
    ...
    suspend fun synchronize(groupId: String): Result<Unit> =
        runCatching {
            ...
            mutex.withLock {
                val securityState =
                    verificationInputs.membershipContext(groupId).security
    ...
    suspend fun onOwnedMembershipChanged(groupId: String): Result<Unit> =
        runCatching {
            mutex.withLock {
                val securityState = verificationInputs.membershipContext(groupId).security
                if (securityState != null) {
                    check(securityState.isLocalAdmin) {
    ...
    suspend fun receiveReceipt(
        ...
        runCatching {
            mutex.withLock {
                ...
                val securityState =
                    verificationInputs.membershipContext(packet.groupId).security
                        ?: error("Group security state was not found")
                check(securityState.isLocalAdmin) {
                ...
                val updated =
                    verificationDataSource.markParticipantVerifiedAdmin(
                        groupId = packet.groupId,
        ...
        runCatching {
            mutex.withLock {
                ...
                val securityState =
                    verificationInputs.membershipContext(packet.groupId).security
                        ?: error("Group security state was not found")
                val localIsAdmin = securityState.isLocalAdmin
                val ownerMemberKey =
                ...
                val latestSnapshot =
                    verificationDataSource.findLatestUpdatedAt(packet.groupId)
                if (
                    latestSnapshot != null &&
    ...
    private suspend fun verifyParticipantAsOwnerLocked(
        ...
        val updated =
            verificationDataSource.markAdminVerifiedParticipant(
                groupId = groupId,
    ...
    private suspend fun verifyOwnerAsParticipantLocked(
        ...
        val membership = verificationInputs.membershipContext(groupId)
            .memberships.singleOrNull()
        ...
        val row =
            verificationDataSource.findPair(
                groupId = groupId,
    ...
    private suspend fun enqueueSnapshotRequestLocked(
        ...
        val membership = verificationInputs.membershipContext(groupId)
            .memberships.singleOrNull() ?: return
