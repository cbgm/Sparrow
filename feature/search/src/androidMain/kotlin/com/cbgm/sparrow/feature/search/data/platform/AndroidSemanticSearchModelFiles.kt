...
class AndroidSemanticSearchModelFiles(
    context: Context
) {
    private val modelDirectory = File(context.filesDir, "semantic-search")
    val modelFile = File(modelDirectory, SemanticSearchModel.MODEL_FILE_NAME)
    val partialFile = File(modelDirectory, "${SemanticSearchModel.MODEL_FILE_NAME}.download")
    ...
    fun ensureDirectory() {
        check(modelDirectory.exists() || modelDirectory.mkdirs()) {
            "Could not create semantic search model directory"
        }
    }
    ...
    fun promoteVerifiedDownload(sha256: String) {
        ...
        check(
            ...
            "Could not install semantic search model"
        }
        hashFile.writeText(sha256)
        check(verifyStoredHash()) {
            "Semantic search model integrity verification failed"
        }
    }
