...
Sparrow is not trying to win by claiming that its cipher is magically stronger than every established messenger.
...
Sparrow keeps end-to-end encryption on the clients too, but additionally splits the server side into multiple
...
!!! warning "Not a blanket claim that Sparrow is safer than Signal"
    Signal is a mature, widely deployed secure messenger with a highly developed cryptographic protocol and
    metadata-reduction work such as Sealed Sender. Sparrow is still an experimental project, has not had a
    completed independent security audit, and currently has a usable Android client only.

    The useful claim is narrower: **Sparrow can reduce some risks created by concentrating routing,
...
flowchart LR
    ...
    subgraph Sparrow[Federated Sparrow]
...
The exact Sparrow pieces are implemented by classes such as:
...
**There is a useful similarity, but Sparrow is not Tor.**
...
Sparrow currently connects a client to **one Community Node at a time**; that node may federate an encrypted
...
flowchart LR
    ...
    subgraph SC[Sparrow message routing]
...
Therefore Sparrow **must not be described as providing Tor anonymity**. Community Nodes and Control Planes can
still observe protocol metadata such as connection IPs/timing, routing activity, envelope sizes and load.

## Sparrow vs Signal vs WhatsApp vs Tor

| Property | Sparrow | Signal | WhatsApp | Tor |
|---|---|---|---|---|
...
| Current Sparrow limitation | Android usable; iOS incomplete | Mature multi-platform product | Mature multi-platform product | Different product category |

## So is Sparrow "safer" than Signal or WhatsApp?

The answer depends on **which threat you mean**.

Sparrow can have an architectural advantage when the threat is:
...
piece of its encryption. Sparrow's differentiator is therefore **not "WhatsApp has no encryption"**. The
difference is primarily that Sparrow is designed to let message transport be spread across independently
operated nodes rather than one provider's infrastructure.

A fair description today is:

> **Sparrow trades some of the maturity of established messengers for a federated, independently hostable
> transport architecture intended to reduce central infrastructure dependence while keeping message content
> end-to-end encrypted.**

## What Sparrow does not protect against yet
