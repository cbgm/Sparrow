...
import com.cbgm.sparrow.server.security.ReplayProtection
...
interface NodeRegistryStorage : AutoCloseable {
    ...
    suspend fun register(descriptor: SparrowNodeDescriptor): RegistrationResult

    suspend fun heartbeat(heartbeat: NodeHeartbeatRequest): RegistrationResult

    suspend fun healthyNodes(): List<SparrowNodeDescriptor>

    suspend fun findHealthy(nodeId: String): SparrowNodeDescriptor?
}
...
class NodeRegistryStore(
    ...
    private data class RegisteredNode(
        val descriptor: SparrowNodeDescriptor,
    ...
    override suspend fun register(descriptor: SparrowNodeDescriptor): RegistrationResult {
        val currentTime = now()
        val rejection =
    ...
    override suspend fun healthyNodes(): List<SparrowNodeDescriptor> {
        val currentTime = now()
        return nodes.values
            .asSequence()
    ...
    override suspend fun findHealthy(nodeId: String): SparrowNodeDescriptor? =
