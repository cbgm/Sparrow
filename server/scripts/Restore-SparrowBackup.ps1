...
if (-not $ValidateOnly -and -not $ConfirmDataLoss) {
    throw "Restore replaces Sparrow databases and identities. Run again with -ConfirmDataLoss."
}
...
function Write-ComposeFailureDiagnostics {
    ...
    Write-Warning "Sparrow service startup failed. Compose state and failing health checks follow."
...
$databaseDefinitions = @(
    [pscustomobject]@{
        ...
        Database = "sparrow_registry"
        User = "sparrow_registry"
    },
    [pscustomobject]@{
        Service = "mailbox-database"
        Database = "sparrow_mailbox"
        User = "sparrow_mailbox"
    },
    [pscustomobject]@{
        Service = "push-database"
        Database = "sparrow_push"
        User = "sparrow_push"
    },
    [pscustomobject]@{
        Service = "federation-database"
        Database = "sparrow_federation"
        User = "sparrow_federation"
    }
)
...
if ($MultiNode) {
    $databaseDefinitions += @(
        [pscustomobject]@{
            ...
            Database = "sparrow_mailbox_b"
            User = "sparrow_mailbox_b"
        },
        [pscustomobject]@{
            Service = "federation-b-database"
            Database = "sparrow_federation_b"
            User = "sparrow_federation_b"
        }
    )
    $identityDefinitions += @(
        [pscustomobject]@{
...
$stagingDirectory = Join-Path ([IO.Path]::GetTempPath()) "sparrow-restore-$([Guid]::NewGuid())"
...
try {
    ...
    if ($RecreateVolumes) {
        ...
        Write-Host "Stopping Sparrow application services."
    ...
    foreach ($database in $databaseDefinitions) {
        ...
        $containerDumpPath = "/tmp/sparrow-restore-$([Guid]::NewGuid()).dump"
        try {
    ...
    if ($BuildImages) {
        Write-Host "Building Sparrow application images before startup."
