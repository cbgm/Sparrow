...
$archivePath = Join-Path $OutputDirectory "Sparrow-backup-$backupTimestamp.zip"
$stagingDirectory = Join-Path ([IO.Path]::GetTempPath()) "sparrow-backup-$([Guid]::NewGuid())"
$databaseManifest = @()
$identityManifest = @()
...
try {
    foreach ($database in $databaseDefinitions) {
        ...
        $containerDumpPath = "/tmp/sparrow-backup-$([Guid]::NewGuid()).dump"
    ...
    if ($RetentionDays -gt 0) {
        ...
        Get-ChildItem -LiteralPath $OutputDirectory -Filter "Sparrow-backup-*.zip" -File |
            Where-Object {
